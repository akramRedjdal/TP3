...
int main (void)
{
	...
	*ptr = 0;
	/*
	  Bon, le bug etait facile a trouver. Mettez la ligne
	  ci-dessus en commentaire pour eliminer le probleme,
	  recompilez, recharger le fichier : tapez la commande file
	  "./gdb-tutorial"" dans gdb et lisez la suite.
	...
	*/
	...
	p2.y = 10;
	/*
	  On peut aussi parcourir des structures en utilisant le print
	  (gdb) p p1 
	  ou en double-cliquant sur la variable sous dd ou en séléctionnant 
	  la variable sous emacs et cliquez dans le menu sur watch
	*/
	printf("Point (%f,%f) \n", p1.x,p1.y); //mettre un break ici
	/* 
	   Tout ca se complique un peu  quand on a des appels de fonctions.
	   Entrez dans la fonction  fact appelee a la ligne suivante 
	   avec la commande "s", on se retrouve la bas !
	*/
